...
| `phase3_confirmed_v7_300h_paired` | confirmed runtime + v7 300h LoRA | **93.1%** | **Promising, not final.** First non-route-table runtime to beat `x6_ensemble` on paired while also improving assisted-OOS to **61.3%** with 5/5 locks. OOS is still machine-assisted and only +0.5 over the prior gate, so continue controlled 3-epoch training and require gold OOS before any 95%+ production claim. |
